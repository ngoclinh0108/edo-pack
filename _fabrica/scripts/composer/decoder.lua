...
function automatons.anime(data)
    ...
    function states.monster()
        ...
        if mtype == 0 then return nil, "Missing monster type" end
    ...
    function states.link()
        ...
        insert(layers,
               MetaLayer.new("link_rating", Parser.get_link_rating(data)))
        return states.atk()
    end
...
function automatons.proxy(data)
    ...
    function states.baseframe()
        local frame
        if Parser.bcheck(data.type, types.SPELL) then
            frame = 2
    ...
    function states.pendulum()
        ...
        local p_scales_ml = MetaLayer.new("pendulum_scales",
                                          Parser.get_scales(data))
    ...
    function states.pendulum_link()
        local function p1(self, t)
            self:set_value(1, self:get_value(1):format(t))
        end
    ...
    function states.monster_desc()
        ...
        local sumtype = Parser.get_sumtype(data);
        sumtype = sumtype and "/" .. sumtype or ""
        local pend = Parser.bcheck(data.type, types.PENDULUM) and "/Pendulum" or
                         ""
        local ability = Parser.get_ability(data);
        ability = ability and "/" .. ability or ""
        local tuner = Parser.bcheck(data.type, types.TUNER) and "/Tuner" or ""
        local effnorm =
            (Parser.bcheck(data.type, types.EFFECT) and "/Effect") or
                (Parser.bcheck(data.type, types.NORMAL) and "/Normal") or ""
        local desc = ("%s%s%s%s%s%s"):format(race, sumtype, pend, ability,
                                             tuner, effnorm)
    ...
    function states.monster_text()
        ...
        if Parser.bcheck(data.type, types.NORMAL) and
            not Parser.bcheck(data.type, types.TOKEN) then
    ...
    function states.serial_code()
        local darkbg = Parser.bcheck(data.type, types.XYZ) and
                           not Parser.bcheck(data.type, types.PENDULUM)
    ...
    if not metalayers then return nil, err end
