...
function s.e1tg(e, tp, eg, ep, ev, re, r, rp, chk)
    ...
    Duel.SetOperationInfo(0, CATEGORY_DESTROY, nil, 1, tp,
...
function s.e1op(e, tp, eg, ep, ev, re, r, rp)
    ...
    local dg = Utility.SelectMatchingCard(HINTMSG_DESTROY, tp, s.e1filter, tp,
                                          LOCATION_HAND + LOCATION_MZONE, 0, 1,
                                          1, nil)
...
function s.e2filter(c)
    ...
    return c:IsCode(83764718) or
               (aux.IsCodeListed(c, 83764718) and
...
function s.e3tg(e, tp, eg, ep, ev, re, r, rp, chk)
    local c = e:GetHandler()
    local g = Duel.GetMatchingGroup(s.e3filter, tp,
                                    LOCATION_GRAVE + LOCATION_REMOVED, 0, nil)
    if chk == 0 then return c:IsAbleToDeck() and #g >= 5 end

    Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TODECK)
    local tg = Duel.SelectTarget(tp, s.e3filter, tp,
                                 LOCATION_GRAVE + LOCATION_REMOVED, 0, 5, 5, nil)
    tg:AddCard(c)
