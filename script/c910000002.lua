...
function s.e1cost(e, tp, eg, ep, ev, re, r, rp, chk)
    ...
    if chk == 0 then
        return not c:IsPublic() and Duel.GetActivityCount(tp, ACTIVITY_SUMMON) == 0
    end
...
function s.e2tg(e, tp, eg, ep, ev, re, r, rp, chk)
    if chk == 0 then
        return Duel.IsExistingTarget(s.e2filter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
    end
...
function s.e2op(e, tp, eg, ep, ev, re, r, rp)
    ...
    if tc:IsFacedown() or not tc:IsRelateToEffect(e) or not tc:IsCanBeFusionMaterial() or tc:IsImmuneToEffect(e) then
        return
    end

    local sc = Utility.SelectMatchingCard(HINTMSG_FMATERIAL, tp, s.e2filter2, tp, LOCATION_EXTRA, 0, 1, 1, nil, e, tp,
        tc):GetFirst()
    if sc then
...
function s.e3cost(e, tp, eg, ep, ev, re, r, rp, chk)
    local c = e:GetHandler()
    if chk == 0 then
        return Duel.GetActivityCount(tp, ACTIVITY_SPSUMMON) == 0 and c:IsReleasable()
    end
...
function s.e3tg(e, tp, eg, ep, ev, re, r, rp, chk)
    ...
    if chk == 0 then
        return Duel.IsExistingMatchingCard(s.e3filter, tp, loc, 0, 1, nil, e, tp) and
