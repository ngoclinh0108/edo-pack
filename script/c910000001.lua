...
function s.e1op(e, tp, eg, ep, ev, re, r, rp)
    ...
    if not c:IsCanBeSpecialSummoned(e, 0, tp, false, false) or
        Duel.GetLocationCount(tp, LOCATION_MZONE) == 0 or c:IsPublic() or
        not Duel.SelectYesNo(tp, 1075) then return end
...
function s.e4val(e, c) return e:GetHandler():GetAttack() * 2 end
