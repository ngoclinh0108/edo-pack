...
function s.initial_effect(c)
    ...
    e2:SetCategory(CATEGORY_EQUIP + CATEGORY_DISABLE + CATEGORY_ATKCHANGE)
...
function s.e2filter(c) return c:IsFaceup() and c:IsType(TYPE_EFFECT) end
...
function s.e2tg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
    ...
    if chk == 0 then
        return e:IsHasType(EFFECT_TYPE_ACTIVATE) and
                   Duel.IsExistingTarget(s.e2filter, tp, LOCATION_MZONE,
...
function s.e3filter(c) return c:IsCode(91697229) and c:IsAbleToHand() end
