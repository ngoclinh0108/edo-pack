...
function s.e1op(e, tp, eg, ep, ev, re, r, rp)
    ...
    local mg = Duel.GetMatchingGroup(aux.TRUE, tp, LOCATION_MZONE +
                                         LOCATION_HAND + LOCATION_DECK +
                                         LOCATION_GRAVE, 0, c)

    local sg = Group.CreateGroup()
    while true do
        local finishable = #sg > 1
        local cancelable = false
        local sc = Group.SelectUnselect(mg, sg, tp, finishable, cancelable, 1, 3)
        if sc and sg:IsContains(sc) then
            sg:RemoveCard(sc)
            mg:AddCard(sc)
        elseif sc and not sg:IsContains(sc) then
            sg:AddCard(sc)
            mg:RemoveCard(sc)
        end
    end
...
-- function s.e1filter(c) return c:IsMonster() and c:IsAbleToRemoveAsCost() end
...
--     local c = e:GetHandler()
--     local g = Duel.GetMatchingGroup(s.e1filter, tp, LOCATION_MZONE +
--                                         LOCATION_HAND + LOCATION_DECK +
--                                         LOCATION_GRAVE, 0, c)
--     if chk == 0 then return g:IsExists(Card.IsSetCard, 1, nil, 0x13a) end

--     local sg = aux.SelectUnselectGroup(g, e, tp, 1, nil, s.e1check, 1, tp,
--                                        HINTMSG_REMOVE)
