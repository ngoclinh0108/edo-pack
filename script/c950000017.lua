...
function s.pe2filter2(c) return c:IsType(TYPE_SPELL + TYPE_TRAP) end
...
function s.pe2tg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
    if chk == 0 then
        return Duel.IsExistingTarget(s.pe2filter2, tp, LOCATION_ONFIELD,
                                     LOCATION_ONFIELD, 1, nil)
    end

    Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_DESTROY)
    local g = Duel.SelectTarget(tp, s.pe2filter2, tp, LOCATION_ONFIELD,
