...
function Divine.DivineHierarchy(s, c, divine_hierarchy, cannot_be_negated)
    ...
    togy:SetOperation(function(e, tp, eg, ep, ev, re, r, rp) Duel.SendtoGrave(e:GetHandler(), REASON_EFFECT) end)
...
function ResetEffectFilter(te, c)
    ...
    if tc == c or tc:IsCode(10000080) or Divine.GetDivineHierarchy(tc) > Divine.GetDivineHierarchy(c) then return false end
