...
function s.e1filter1(c)
    return not c:IsCode(id) and c:IsSetCard(0x1017) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end

function s.e1filter2(c)
    return c:IsSummonable(true, nil) and c:IsType(TYPE_TUNER)
end
...
function s.e1op(e, tp, eg, ep, ev, re, r, rp)
    ...
    if Duel.IsExistingMatchingCard(s.e1filter2, tp, LOCATION_HAND, 0, 1, nil) and
        ...
        local tc = Utility.SelectMatchingCard(HINTMSG_SUMMON, tp, s.e1filter2, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1,
            1, nil):GetFirst()
        if tc then
