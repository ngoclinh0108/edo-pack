...
function s.e3op(e, tp, eg, ep, ev, re, r, rp)
    ...
    if Duel.IsPlayerAffectedByEffect(tp, CARD_BLUEEYES_SPIRIT) or
        ...
        not Duel.IsExistingMatchingCard(s.e3filter2, tp, LOCATION_GRAVE,
                                        LOCATION_GRAVE, 2, nil, e, tp) then
        return
    end

    Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
    local g = Duel.SelectMatchingCard(tp, s.e3filter2, tp, LOCATION_GRAVE,
...
function s.e3gyop(e, tp, eg, ep, ev, re, r, rp)
